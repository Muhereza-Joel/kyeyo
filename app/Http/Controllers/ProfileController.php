...
class ProfileController extends Controller
{
    public function renderUserProfile()
    {
        ...
        return Inertia::render('Profile/UserProfile', [
            ...
            'profile' => $profile, // Include profile with logo_url
            'logo_url' => $profile ? $profile->getFirstMediaUrl('avator_images') : null,
            'cover_image' => $profile ? $profile->getFirstMediaUrl('profile_cover_images') : null,
            'avator' => $profile ? $profile->getFirstMediaUrl('avator_images') : null,
        ]);
    }
    /**
    ...
    public function updateBioData(Request $request)
    {
        ...
        // Validate logo and cover image only if they are files
        if ($request->hasFile('logo')) {
            $validated['logo'] = $request->validate([
                'logo' => 'image|mimes:jpg,jpeg,png|max:2048',
            ])['logo'];
        }

        if ($request->hasFile('cover_image')) {
            $validated['cover_image'] = $request->validate([
